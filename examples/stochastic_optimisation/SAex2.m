...
gam = 2;      % initial random walk step length
Niter = 5000;    % number of iterations

x = SAfunc(@(y) exp(-10*(y(1)^2 - y(2))^2 - (y(2) - 0.25)^4),x1,gam,Niter);
...
[V,D] = eigs(covar);
w = diag(D);
%e1 = [mu(1) - 5*w(1)*V(1,1), mu(1) + 5*w(1)*V(1,1);mu(2) - 5*w(1)*V(2,2), mu(2) + 5*w(1)*V(2,2) ];
%e2 = [mu(1) - 5*w(2)*V(1,2), mu(1) + 5*w(2)*V(1,2);mu(2) - 5*w(2)*V(2,2), mu(2) + 5*w(2)*V(2,2) ];
